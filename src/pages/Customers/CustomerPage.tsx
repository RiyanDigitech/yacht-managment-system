...
function CustomerPage() {
  ...
  const { useGetCustomer,useDeleteCustomer } = CustomerService();
  const { data:addons, isLoading } = useGetCustomer( page, pageSize);
  //console.log("addons", addons);
  const deleteAddons = useDeleteCustomer();
...
const columns = [
  ...
  return (
    <div className="p-4   rounded-[10px] ">
      ...
      <CreateCustomer isOpen={open} onClose={() => setOpen(false)} />
      <EditCustomer
        open={editingAddons}
