import { Input, Pagination, Spin, Table } from "antd";
import { useState } from "react";
import "../../index.css";
import LeadService from "@/services/lead-management/lead-service";
import { Lead } from "@/lib/types/lead";
import tokenService from "@/services/token.service";
import FranchiseLeadListTableColumns from "@/components/modules/lead/franchise-lead-list-table";

const FranchiseLeadList = () => {
  const [limit, setFranchiseLimit] = useState<number>(5);
  const [page, setFranchiseCurrentPage] = useState<number>(1);
  const [query, setFranchiseQuery] = useState<string | undefined>(undefined);
  const { useFetchTargetedFranchiseLead } = LeadService();
  const email = tokenService?.getLastUserData()?.email;
  const { data: leaddata, isLoading: isleaddataLoading } =
    useFetchTargetedFranchiseLead(limit, page, email, query);

  const [selectedRowKeys, setSelectedRowKeys] = useState<React.Key[]>([]);
  const rowSelection = {
    onChange: (selectedRowKeys: React.Key[]) => {
      console.log("selectedRowKeys: ", selectedRowKeys);
      setSelectedRowKeys(selectedRowKeys);
    },
  };

  const handlePageChange = (page: number, pageSize?: number) => {
    setFranchiseCurrentPage(page);
    if (pageSize) setFranchiseLimit(pageSize);
  };
  const totalResults = leaddata?.total || 0;
  const showTotal = (total: number) =>
    `Page ${Math.min(page * limit, totalResults)} of ${total} Results`;
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setFranchiseQuery(e.target.value);
  };
  console.log(email, "last user email");

  return (
    <div className="p-5">
      <div className="flex justify-between items-center flex-wrap md:flex-nowrap">
        {" "}
        <div className="border-b-[1px]">
          <Input
            // size="large"
            placeholder="Search..."
            prefix={<img src="/preffixsearch.png" />}
            onChange={handleSearchChange}
            className="border-none bg-[#f8fafc] pl-2"
          />
        </div>
      </div>
      <Table<Lead>
        columns={FranchiseLeadListTableColumns()}
        dataSource={leaddata?.data?.map((team) => ({
          ...team,
          key: team.id,
        }))}
        loading={{
          spinning: isleaddataLoading,
          indicator: (
            <div className="flex items-center justify-center">
              <Spin
                className="custom-table-spinner"
                size="large"
                style={{ color: "#008444" }}
              />
            </div>
          ),
        }}
        pagination={false}
        rowSelection={{
          type: "checkbox",
          ...rowSelection,
        }}
        className="overflow-auto border-[1px] rounded-lg mt-7  bg-white franchise-table"
      />{" "}
      <div className=" flex justify-between items-center mt-4  flex-col md:flex-row">
        <div className="text-sm text-gray-600 mb-5 md:mb-0">
          {showTotal(totalResults)}
          {/* <button className="ml-2  cursor-pointer">All</button> */}
        </div>
        <Pagination
          current={page}
          pageSize={limit}
          total={totalResults}
          onChange={handlePageChange}
          showSizeChanger={false} // Enable page size changer
          size="small"
        />
      </div>
    </div>
  );
};

export default FranchiseLeadList;
